...
export default function COCerca() {
    ...
    <div style={{ ...S.page, justifyContent:"center", padding:"40px 24px", textAlign:"center" }}>
      ...
      <div className="fu" style={{ marginBottom:32 }}>
        ...
        <div style={{ fontSize:11,color:"#7fffd4",opacity:0.5,lineHeight:1.8,fontFamily:"sans-serif",maxWidth:280,margin:"0 auto 32px" }}>
          Una probada de las apps del nuevo sistema de comunicación que <strong style={{ color:"#7fffd4",opacity:1 }}>no depende de internet</strong>.
        </div>
        <div style={{ fontSize:9,color:"#2a5a3a",letterSpacing:3,marginBottom:32 }}>DESCENTRALIZADO · ENCRIPTADO · LIBRE</div>
        <button style={{ ...S.btn, maxWidth:320, margin:"0 auto" }} onClick={() => setScreen("registro")}>ENTRAR</button>
      </div>
    </div>
  );
          ...
          <div className="fu">
            ...
            {regPresType==="foto" && (
              <div style={{ marginBottom:16 }}>
                ...
                <div style={{ border:"1px dashed #1a4a2a", borderRadius:12, padding:"20px", textAlign:"center", marginBottom:12, cursor:"pointer" }} onClick={()=>document.getElementById("photoInput").click()}>
                  {regPhotoURL ? <img src={regPhotoURL} alt="" style={{ width:80,height:80,borderRadius:"50%",objectFit:"cover",border:"2px solid #4ecda4" }}/> : <div style={{ color:"#2a6a4a", fontSize:12, letterSpacing:1 }}>📸 SUBIR FOTO<br/><span style={{ fontSize:10, opacity:0.6 }}>tocá para elegir</span></div>}
                </div>
            ...
            {regPresType==="video" && (
              <div style={{ border:"1px dashed #1a4a2a", borderRadius:12, padding:"24px", textAlign:"center", marginBottom:16 }}>
                ...
                <div style={{ color:"#2a6a4a", fontSize:10, lineHeight:1.6 }}>Grabá un video corto presentándote.</div>
                <button style={{ ...S.btn, marginTop:16 }}>🎥 GRABAR VIDEO</button>
              </div>
            )}
            <div style={{ display:"flex", gap:10, marginTop:8 }}>
              <button style={{ ...S.btnOutline, width:"auto", padding:"12px 20px", marginTop:0 }} onClick={()=>setRegStep(2)}>← ATRÁS</button>
              <button style={{ ...S.btn, marginTop:0, opacity:regPresType?1:0.4 }} disabled={!regPresType}
                onClick={()=>{ setMyProfile({ name:regName, age:parseInt(regAge), frase:regFrase, busca:regBusca, presType:regPresType, photo:regPhotoURL||"https://i.pravatar.cc/150?img=33", mensaje:regMensaje }); setScreen("radar"); }}>
                ENTRAR AL RADAR ✓
              </button>
            </div>
          </div>
  ...
  return (
    <div style={{ ...S.page }}>
        ...
        {visibleUsers.map(user => {
          ...
          return (
            <div key={user.id} className="udot" onClick={() => setSelected(isSel ? null : user)}
              ...
              <img src={user.photo} alt={user.name} style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }} onError={e=>{e.target.style.display="none";e.target.nextSibling.style.display="flex";}}/>
      ...
      {selected && (
        <div className="panel" style={{ background:"#0d1a14", border:"1px solid #1a4a2a", borderRadius:16, padding:"20px", width:"100%", boxSizing:"border-box", padding:"16px 18px" }}>
